...
使用 0.35 m 路径净空、最长 1.5 m 分段目标。发现机器人先驶向自己的目标附近集合位，其余
机器人再按目标背侧到近侧串行进场，防止已停机器人挡住后来者。只有所有机器人位置误差不
超过 0.35 m、线速度不超过 0.05 m/s、角速度不超过 0.10 rad/s 并连续保持 5 个仿真秒，
协调器才发布 `COMPLETE`。
...
goal，并在约 5.2 秒后到达自己的目标附近集合位；随后 tb2 进场，117.9 秒进入 `COMPLETE`，
两机器人最终误差为 0.215/0.206 m，碰撞为 0。
