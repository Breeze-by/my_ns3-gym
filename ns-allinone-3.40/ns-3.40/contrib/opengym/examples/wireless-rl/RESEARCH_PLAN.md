...
截至 2026-09-17，P1 已建立固定 seed、批量运行、真值覆盖率、路径、碰撞和搜索重叠评估，
P2A 已建立可重复的目标确认 MVP，P2B 已完成并通过理想直达事件下的停止探索、安全集合和
稳定完成闭环，P2C 已完成本地能量、安全返航和充电恢复并等待用户验收。仍有以下限制：

- 同机 ROS 2/DDS 仍是理想网络，没有 Wi-Fi 排队、丢包和干扰；
- 中央协调器和 `merge_map` 直接读取机器人地图、odom、TF，并直接下发 Nav2 action；
- 当前 1–3 号机器人 Nav2 全局代价图直接读取中央 `/merge_map`，融合地图下行也未经过网络；
- P2A 检测器集中读取 Gazebo 真值，尚未形成“机器人本地产生、经网络交付”的检测消息；
- 当前集合闭环仍依赖同机 ROS 理想直达事件和中央直接 Nav2 action，尚未经过统一 gateway；
- 电池/返航/充电已形成理想通信组件闭环，但尚未完成 P2D 跨场景完整基线，通信量和 AoI
  仍未进入闭环；
