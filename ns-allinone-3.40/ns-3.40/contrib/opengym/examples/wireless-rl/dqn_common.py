...
USER_NUM = 5
FEATURES_PER_USER = 3
STATE_DIM = USER_NUM * FEATURES_PER_USER
ACTION_DIM = USER_NUM
CQI_MAX = 10.0
QUEUE_MAX = 100.0
DELAY_MAX = 20.0
...
def normalize_observation(obs):
    ...
    state[0::FEATURES_PER_USER] /= CQI_MAX
    state[1::FEATURES_PER_USER] /= QUEUE_MAX
    state[2::FEATURES_PER_USER] /= DELAY_MAX
    return state


def split_observation(obs):
    obs = np.asarray(obs, dtype=np.float32)
    return (obs[0::FEATURES_PER_USER],
            obs[1::FEATURES_PER_USER],
            obs[2::FEATURES_PER_USER])
...
def summarize_rows(rows):
    ...
    summary = {
        ...
        "average_total_delay": float(np.mean(total_delays)),
        "average_deadline_misses": float(np.mean(deadline_misses)),
        "final_current_queue": float(current_queues[-1]),
        "final_reward_queue": float(reward_queues[-1]),
        "final_total_delay": float(total_delays[-1]),
        "final_deadline_misses": float(deadline_misses[-1]),
...
def heuristic_action_from_raw_obs(obs, strategy="max_service"):
    ...
    if strategy == "delay_aware":
        score = cqi * queue + 20.0 * delay
        if np.max(score) <= 0:
            score = cqi
        return int(np.argmax(score))

    if strategy == "max_delay":
        if np.max(delay) <= 0:
            if np.max(queue) > 0:
...
def make_synthetic_training_batch(batch_size, rng, strategy="max_service"):
    ...
    delay = rng.integers(0, int(DELAY_MAX) + 1, size=(batch_size, USER_NUM))
    delay = np.where(queue > 0, delay, 0)

    raw_states = np.empty((batch_size, STATE_DIM), dtype=np.float32)
    raw_states[:, 0::FEATURES_PER_USER] = cqi
    raw_states[:, 1::FEATURES_PER_USER] = queue
    raw_states[:, 2::FEATURES_PER_USER] = delay
    ...
    states[:, 0::FEATURES_PER_USER] /= CQI_MAX
    states[:, 1::FEATURES_PER_USER] /= QUEUE_MAX
    states[:, 2::FEATURES_PER_USER] /= DELAY_MAX
...
def build_step_row(agent, seed, episode, step, obs, action, reward, total_reward, done, info):
    ...
    row = {
        ...
        "delay": " ".join(str(int(value)) for value in delay),
    }
