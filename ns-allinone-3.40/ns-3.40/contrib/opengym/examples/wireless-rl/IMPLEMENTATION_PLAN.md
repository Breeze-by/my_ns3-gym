...
- P2A 已加入不参与探索决策的 Gazebo 真值目标检测：同时满足距离、水平视场、静态墙体
  视线和连续帧规则后发布 `FOUND`，并已通过用户验收；电池、充电、集合、显式消息和
  Wi-Fi 场景仍未实现；
- P2B 前路线审计补充了 P2D 完整任务集成门、P4A 时间/包级对账门和正式统计规则，详见
  `report/20260917_roadmap_audit.md`；当前下一检查点仍是 P2B；
...
| P2B | 集合状态机 | 权威任务状态机、停止探索、独立 staging poses、保持判定 | 3 机器人收到理想直达目标事件后取消探索，在不同安全位姿以位置误差≤0.35 m、线/角速度≤0.05 m/s、0.10 rad/s 连续稳定 5 秒并只在此时进入 `COMPLETE` | 待开始 |
