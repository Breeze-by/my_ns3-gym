...
- P1B 已加入只读真值评估器；P1C 修复定位、导航、融合和协同探索基础后，
  seeds 101/202/303 均在 180 秒内超过 90% 正确自由空间覆盖率，等待用户验收；
- readiness gate 的实现锚点为 `00c0de2`；最近两机器人 headless smoke 通过，
  正式三 seed P1C 结果来自未改变任务逻辑的前一提交 `fd5f6ff`；
...
| P1C | 理想通信探索基线 | 固定 world、共同出生/充电区、seed、90% 覆盖率和 180 秒超时的批量入口与日期报告 | 2 机器人多个独立 seed 可重复达到 90%，导航和碰撞结果完整记录 | 待用户验收 |
...
因此 P1C 采用 90%/180 秒作为当前退出条件；95% 仍是更高里程碑。历史阈值实验见
`report/20260916_p1c.md`，本轮基础修复与最终证据见
`report/20260916_p1c_foundation.md`。等待用户验收后再进入 P2。
