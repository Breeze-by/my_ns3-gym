...
- P2B 前路线审计补充了 P2D 完整任务集成门、P4A 时间/包级对账门和正式统计规则，详见
  `report/20260917_roadmap_audit.md`；当前检查点 P2C 已验收，P2D 尚未开始；
...
验收后的操作可视化补充不改变控制或评分：Gazebo 中用无 collision 的半透明圆盘标出共同
起始/充电区、每台机器人充电位、目标检测半径和状态机发布的集合位姿；独立 Qt 状态栏只读
