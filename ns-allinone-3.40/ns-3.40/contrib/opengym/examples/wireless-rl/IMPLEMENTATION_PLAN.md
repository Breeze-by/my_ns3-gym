...
`RALLY`。不能把所有机器人发送到目标坐标，也不能因为某台失败而从 required set 删除它。
...
0.8 m；到达路径使用 0.35 m 净空和不超过 1.5 m 的分段目标，并按目标背侧到近侧的顺序
进场，避免先停靠机器人阻塞后来者。协调器发布 `/rally_assignments` 和明确的
...
安全/稳定性反例撤回，详见 `report/20260917_p2b_audit.md`。P2B 现为 `待用户验收`，尚未
开始 P2C。
