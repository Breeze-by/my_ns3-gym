...
- P1B 已加入只读真值评估器；P1C 根据 90%/600 秒和 80%/300 秒的多轮失败证据，
  将验收口径修订为 75%/300 秒；最终 seeds 101–103 为 3/3 成功，等待用户验收；
...
| P1C | 理想通信探索基线 | 固定 world、共同出生/充电区、seed、75% 覆盖率和 300 秒超时的批量入口与日期报告 | 2 机器人多个独立 seed 可重复探索，成功和失败均记录 | 待用户验收 |
...
90% 正确自由空间覆盖率和 600 仿真秒超时；2026-09-16 根据多轮实验和用户对等待时间的反馈，
将 P1C 工程验收口径修订为 75% 和 300 仿真秒。80%/90%/95% 仍作为更高覆盖里程碑报告，
不能把 75% 结果描述成完整地图覆盖。

截至 2026-09-16，控制器不再只保留最大的 8 个前沿组，避免后期有效小组永远无法参与评分。
在相同代码下，80%/300 秒批次为 0/3（75.7%、75.8%、79.7%），确认 80% 仍不稳定；
最终 75%/300 秒批次为 3/3 成功，首次达标时间 70.7–91.2 秒、零碰撞。完整尝试、失败方案和
阈值依据见 `report/20260916_p1c.md`。P1C 已达到修订后的工程退出条件，等待用户验收后再进入 P2。
