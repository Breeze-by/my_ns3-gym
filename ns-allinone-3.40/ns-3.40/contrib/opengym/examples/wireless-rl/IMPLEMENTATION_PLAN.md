...
- P1B 已加入只读真值评估器；P1C 已打通融合地图前沿、共享全局导航、分段路径和
  ...
  并已通过用户验收；
- P2A 已加入不参与探索决策的 Gazebo 真值目标检测：同时满足距离、水平视场、静态墙体
  视线和连续帧规则后发布 `FOUND`；电池、充电、集合、显式消息和 Wi-Fi 场景仍未实现；
...
| P2A | 目标检测与确认 MVP | Gazebo 真值评估、视场/距离/遮挡、连续帧确认 | 目标不可见时不触发，可见并满足规则时进入 `FOUND` | 待用户验收 |
...
任一不可见帧会清零该机器人的确认计数。该检测节点只发布 `/task_state` 和
`/target_detection`，不修改探索目标、Nav2、地图、传感器、安全距离或评分规则；集合动作留给
P2B。
